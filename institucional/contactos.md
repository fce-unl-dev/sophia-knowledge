# Contactos y Canales Oficiales (FCE-UNL)

## Para qué sirve

- Documento del sector **Institucional**: canales de contacto oficiales de la Facultad de
  Ciencias Económicas (FCE-UNL). Dato de alta prioridad y baja volatilidad.
- Es la referencia para responder "¿a dónde escribo / a qué WhatsApp / a qué mail por X tema?".

## Canales principales

- **Posgrado** (Maestrías, Especializaciones, Doctorados, Diplomaturas): posgrado@fce.unl.edu.ar — WhatsApp **+54 9 342 449 1939**
- **Difusión / Ingreso / Comunicación**: difusion@fce.unl.edu.ar
- **Bedelía / Reservas / Trámites generales de alumnos**: bedelia@fce.unl.edu.ar
- **Tesorería** (pagos, cuotas, facturas): tesoreria@fce.unl.edu.ar — Tel. (0342) 4571179 interno 145
- **Secretaría Académica**: academica@fce.unl.edu.ar
- **Relaciones Internacionales**: internacional@fce.unl.edu.ar

## Advertencias para Sophia

- El número **+54 9 342 449 1939** es WhatsApp de Posgrado. El número **(0342) 4571179 / 3425818810**
  es teléfono de LÍNEA (Bedelía/Tesorería): NO lo presentes como WhatsApp ni sugieras enviar
  audios o mensajes ahí.
- No brindes teléfonos celulares ni correos personales de personas físicas (R12). Estos correos
  son institucionales de área y sí son públicos.

## Fuentes consultadas

- Organización de Gestión: https://www.fce.unl.edu.ar/institucional/gobierno/organizacion-de-gestion
- Datos operativos de gestión administrativa FCE-UNL.

---

**Última revisión**: 2026-06-04
**Revisión humana**: confirmada
